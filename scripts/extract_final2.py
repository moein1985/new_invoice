#!/usr/bin/env python3
"""Extract ALL events - robust encoding handling"""
import re, json, codecs

html = open('/tmp/timeir_events.html', 'r', encoding='utf-8').read()
chunks = re.findall(r'self\.__next_f\.push\(\[1,"(.*?)"\]\)', html)

def decode_rsc_chunk(chunk):
    """Decode RSC chunk with proper Unicode handling"""
    # Replace \uXXXX sequences manually
    def replace_unicode(match):
        cp = int(match.group(1), 16)
        return chr(cp)
    
    # First handle \uXXXX
    result = re.sub(r'\\u([0-9a-fA-F]{4})', replace_unicode, chunk)
...
for i, chunk in enumerate(chunks):
    ...
    decoded = decode_rsc_chunk(chunk)
